...
export default function LoginForm({ch1}) {
    ...
    const submitForm = async()=>{
      ...
      await farawin.testLogin(mobile, pass, (res) => alert(res.message));
    }
    }
    ...
    return (
      ...
      >
        <div className="bg-white w-[280px] h-[500px] rounded-xl flex flex-col p-6">
          ...
          className="text-sm text-end mt-2">Forgot password?</button>
          ...
          <button 
          type="button"
          onClick={()=>{ch1("register")}}
          className="text-md">SIGN UP</button>
        </div>
      </div>
    )
}
