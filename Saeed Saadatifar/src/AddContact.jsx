...
export default function AddContact({ setActive, setC }) {
  const [username, setUsername] = useState("");
  const [name, setName] = useState("");
  const [firstUseUserame, setFirstUseUserame] = useState(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-14 text-2xl">اضافه کردن مخاطب</div>
        {
          //#region FirstInput
        }
        <div className="relative w-full">
          ...
          <label
            htmlFor="floating_outlined"
            className={`${
              userNameValid() || firstUseUserame ? "hidden" : ""
        ...
        <button
          onClick={() => {
            if (userNameValid() && nameValid()) {
              farawin.testAddContact(username, name, (res) => {
          ...
          className={`${
            !userNameValid() || !nameValid() ? "cursor-not-allowed" : ""
          } py-4 px-12 border-[1px] border-[#21242B] rounded-lg mt-2 ${
            !userNameValid() || !nameValid() ? "opacity-30" : ""
          } ${
            userNameValid() && nameValid() ? "hover:bg-[#21242B]" : ""
          } transition-[all,2s]`}
        >
          اضافه گردد
        </button>
